...
class Plugin(chatMod.chatMod):
    ...
    def query(self, user, channel, msg):
        ...
        if number < chance:
            ...
            self.bot.getNamedService('scheduler').callLater(delay, self.bot.sendmsg, user, reply, "UTF-8")
    def msg(self, user, channel, msg):
        ...
        if reply:
            ...
            if israndom:
                ...
                self.bot.getNamedService('scheduler').callLater(delay, self.bot.sendmsg, channel, reply, "UTF-8")
            elif number < chance: #apply answerPercent only on answers
                #self.bot.sendmsg(channel, user+": "+reply, "UTF-8")
                self.bot.getNamedService('scheduler').callLater(delay, self.bot.sendmsg, channel, user+": "+reply, "UTF-8")
