...
class Plugin(chatMod.chatMod):
	...
	def command(self, user, channel, command, options):
		...
		self.time = time.time()
		if command == "kurs":
			d=urlutils.download("http://de.old.finance.yahoo.com/d/quotes.csv?s="+urllib.quote_plus(options)+"&f=nsl1d1t1cohgvc4x&e=.csv")
			d.addCallback(self.parseKurs, channel)
			d.addErrback(self.error, channel)
		if command == "wkn":
			d=urlutils.download("http://de.finsearch.yahoo.com/de/index.php?s=de_sort&nm="+urllib.quote_plus(options)+"&tp=S")
			d.addCallback(self.parseWKN, channel)
			d.addErrback(self.error, channel)
	
	def error(self, failure, channel):
		self.bot.sendmsg(channel, "Error while fetching data: "+failure.getErrorMessage())
